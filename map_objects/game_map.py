...
class GameMap:
    ...
    def place_entities(self, room, entities):
        ...
        monster_chances = {
            'drone': 80,
            's-bot': from_dungeon_level([[10, 2], [15, 3], [30, 5], [60, 7]], self.dungeon_level)
        }
        ...
        for i in range(number_of_monsters):
            ...
            if not any([entity for entity in entities if entity.x == x and entity.y == y]):
                ...
                if monster_choice == 'drone':
                    fighter_component= Fighter(hp=20, defense=0, power=4, xp=35)
                    ai_component = BasicMonster()

                    monster = Entity(x, y, 'd', libtcod.yellow, 'Drone', blocks=True, render_order=RenderOrder.ACTOR, fighter=fighter_component, ai=ai_component)
                else:
                    fighter_component= Fighter(hp=30, defense=2, power=8, xp=100)
                    ai_component = BasicMonster()

                    monster = Entity(x, y, 'S', libtcod.orange, 'Security Drone', blocks=True,render_order=RenderOrder.ACTOR, fighter=fighter_component, ai=ai_component)
